...
# Import PDF QA tool
from pdf_tools import load_pdf_agent
...
if ("pdf_agent" not in st.session_state) or (getattr(st.session_state, "_last_key", None) != google_api_key):
    st.session_state.pdf_agent = load_pdf_agent("bphn.pdf", google_api_key)

@tool
def ask_pdf(question: str):
    """
    Answer questions based on the contents of the loaded PDF (KUHP).
    """
    return st.session_state.pdf_agent(question)
...
if ("agent" not in st.session_state) or (getattr(st.session_state, "_last_key", None) != google_api_key):
    try:
        ...
        # Tambahkan tool ask_pdf ke daftar tools
        st.session_state.agent = create_react_agent(
            model=llm,
            tools=[get_schema_info, execute_sql, ask_pdf],
            prompt="""You are a helpful assistant that can answer questions about sales data using SQL, and answer legal questions from the KUHP PDF using the ask_pdf tool.

            IMPORTANT: 
            - For sales data, use get_schema_info and execute_sql as described.
            - For legal questions about KUHP, use the ask_pdf tool to answer based on the PDF content.
            ...
            For KUHP questions:
            - Use the ask_pdf tool to answer based on the PDF content.
            - Do not make up legal information; always use the PDF as source.
